...
class HouseSearcher(object):
    def __init__(self, max_price=default_max_price, min_area=default_min_area,
                 min_rooms=default_min_rooms, max_rooms=default_max_rooms):
    ...
    def build_query_url(self, page_number=1, max_price=default_max_price):
        return CASA_SAPO_URL_BASE % (
            self.max_price,
            self.min_area,
            ",".join([str(x) for x in xrange(8)][self.min_rooms:self.max_rooms+1]),
            page_number)
        ...
        for filepath in savefiles:
            with open(filepath) as saved_data_file:
                ...
                for entry in entries:
                    loaded_listing = HouseListing(entry["url"],
    ...
    def process_casa_sapo_page(self, page_number=1):
        ...
        for result in page_results:
            try:
                ...
                location = location.replace("ã", "a").replace("ç", "c").replace("õ", "o")\
                    .replace("à", "a").replace("á", "a").replace("â", "a")\
                    .replace("é", "e").replace("í", "i").replace("ó", "o")\
                    .replace("ú", "u").encode("ascii", "replace")
                ...
                price = result.select('div[class="searchPropertyPrice"]')[0].select('span')[0].getText()\
                ...
                house_listing = HouseListing(url, date_added, location, price,
                                             property_type, rooms, state, useful_area, gross_area)
                if house_listing not in self.house_listings:
                    print "NEW!"
                    print house_listing, "\n"
                    new_entries_count += 1
